...
echo "Uploading computerextensionattributes/JMF Deploy - Verify APNs.xml"
curl -sS -H "Authorization: Bearer $authorizationToken" "$jamfpro_url/JSSResource/computerextensionattributes" -H "Content-Type: application/xml" -X "POST" --data-binary "@$baseDir/computerextensionattributes/JMF Deploy - Verify APNs.xml"
sleep 20
echo ""

echo "Uploading computergroups/JMF Deploy - Verify APNs = Pass.xml"
curl -sS -H "Authorization: Bearer $authorizationToken" "$jamfpro_url/JSSResource/computergroups" -H "Content-Type: application/xml" -X "POST" --data-binary "@$baseDir/computergroups/JMF Deploy - Verify APNs = Pass.xml"
sleep 20
echo ""

echo "Uploading computergroups/JMF Deploy - Verify APNs = Fail.xml"
curl -sS -H "Authorization: Bearer $authorizationToken" "$jamfpro_url/JSSResource/computergroups" -H "Content-Type: application/xml" -X "POST" --data-binary "@$baseDir/computergroups/JMF Deploy - Verify APNs = Fail.xml"
